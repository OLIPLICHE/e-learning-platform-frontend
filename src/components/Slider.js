...
const Arrow = styled.div`
    ...
    pointer-events: ${(props) => (((props.slideIndex === 0 && props.direction === 'left')
     || ((props.slideIndex === props.coursesLength - props.size) && props.direction === 'right')) && 'none')};
    width: 80px;
    height: 50px;
...
const Slider = () => {
  ...
  const handleNext = (direction) => {
    if (direction === 'left' || slideIndex === courses.length - itemsSize) {
  ...
  return (
    <Container>
      <Arrow direction="left" size={itemsSize} onClick={() => handleNext('left')} slideIndex={slideIndex} coursesLength={courses.length}>
        <ArrowLeftOutlined style={{ fontSize: '3rem', color: '#e6e6e6' }} />
      </Arrow>
      <Wrapper slideIndex={slideIndex}>
        {courses.slice(slideIndex, slideIndex + itemsSize).map((item) => (
          <CourseCard key={item.id} item={item} />))}
      </Wrapper>
      <Arrow direction="right" size={itemsSize} onClick={() => handleNext('right')} slideIndex={slideIndex} coursesLength={courses.length}>
