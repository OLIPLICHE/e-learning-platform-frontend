...
const RESERVATIONS_API_ENDPOINT = 'http://localhost:4000/api/v1/reservations';
const CITIES_API_ENDPOINT = 'http://localhost:4000/cities';
...
export const getReservations = createAsyncThunk(
  'redux/courses/getReservations.js',
...
export const addReservation = createAsyncThunk(
  'redux/courses/addReservation.js',
  async (payload, { rejectWithValue }) => {
    const rcoursesJwt = JSON.parse(localStorage.getItem('rcourses_jwt'));
    const token = (rcoursesJwt) ? rcoursesJwt.token : null;
    const config = jsonTypeConfig(token);
...
export const cancelReservation = createAsyncThunk(
  'redux/courses/removeReservation.js',
  async (payload, { rejectWithValue }) => {
    const { token } = JSON.parse(localStorage.getItem('rcourses_jwt'));
    const config = jsonTypeConfig(token);
    try {
